...
# =================
APP_VERSION="0.01"
APP_NAME="A-Team Ventoy + SteamOS Repair / Install Image Installer | Version: ${APP_VERSION}" 
...
echo ""
bash Ventoy2Disk.sh -I -r 7900 -g "$DRIVE"

# ===========================================
# NAME VENTOY CUSTOM IMAGE STORAGE PARTITION
# ===========================================
echo ""
echo "Naming Ventoy Image Storage Partition..."
exfatlabel "${DRIVE}1" "Vtoy Images" 2>/dev/null
echo ""

# =====================================
# ADD VENTOY CUSTOM STEAMOS GRUB ENTRY
# =====================================
ventoy_steamos_grub_entry

# ===============================
# ADD CUSTOM VENTOY A-TEAM THEME
# ===============================
echo ""
...
mkdir -p "$MNT"
mount "$PART" "$MNT" || {
    echo "** ERROR ** --> Themes Copy Failed: $PART"
    echo ""
    echo "Press ENTER To Exit"
    read
    exit 1
...
# =====================================
# ADD CUSTOM A-TEAM VENTOY JSON CONFIG
# =====================================
echo ""
...
mkdir -p "$MNT"
mount "$PART" "$MNT" || {
    echo "** ERROR ** --> Custom Json Copy Failed: $PART"
    echo ""
    echo "Press ENTER To Exit"
    read
    exit 1
...
mkdir -p "$MNT/ventoy"
# copy custom theme
cp -r A-Team/ventoy.json "$MNT/ventoy/"
sync
umount "$MNT"
rmdir "$MNT"
echo "--> DONE."
...
mkdir -p "$MNT"
mount "$PART" "$MNT" || {
    echo "** ERROR ** --> Ventoy Grub Patch Failed: $PART"
    echo ""
    echo "Press ENTER To Exit"
    read
    exit 1
